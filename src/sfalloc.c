...
int main(int argc, char *argv[]) {
    ...
    *value2 = VALUE2_VALUE;
    ...
    printf("Coalesce: %lu\n", (long)in.coalesce);
    printf("Frees: %lu\n", (long)in.frees);
    printf("Allocatios: %lu\n", (long)in.allocations);

    printf("=== Test8: realloc from long to int ===\n");
    void* ptr = sf_malloc(sizeof(long));
    sf_header* current_header = GO_DOWN(ptr);
    sf_blockprint((void*) current_header);
    int old_block_size = SHIFT_BACK(current_header->block_size);
    printf("Old size: %d\nOld padding: %d\n", old_block_size, current_header->padding_size);
    void* new_ptr = sf_realloc(ptr, sizeof(int));
    current_header = GO_DOWN(new_ptr);
    sf_blockprint((void*) current_header);
    printf("Old size: %d\nOld padding: %d\n", 
        SHIFT_BACK(current_header->block_size), current_header->padding_size);

    printf("=== Test9: realloc from 100 to 150 ===\n");
